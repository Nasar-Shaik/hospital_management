...
| **Next action**        | **AWAITING REVIEW.** Stage A (web) executed 2026-08-19 → seven defects (`RISK_REGISTER.md` §0 D14–D20; `TESTING.md` §11b). **All seven are now fixed**, plus **D21** — an audit entry that could never recompute its own hash, found while fixing D17, so the chain would have reported a tamper on an entry nobody touched. D18 moved the queue's patient identity server-side (`EncounterRow`), which also deleted a per-row fetch on the phone; D19 asks which site a branch-stamped write belongs to BEFORE the form rather than on submit; D20 gates the navigation on the hospital's edition (`/auth/me` now carries `features`) and stops two pages rendering a refusal beside an empty module. Server enforcement is unchanged in both. Mobile (`M2` 0/61, `M3` 0/45) still needs hardware and eyes. **C2 is approved in direction, not for implementation.** |
