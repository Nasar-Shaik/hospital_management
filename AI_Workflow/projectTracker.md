...
**Two rows are known not to pass before anyone picks up a phone** (recorded as K4-01 and K4-02 in
the checklist): the server sends `priority: high|default` but the app creates a single Android
channel at HIGH importance, and on Android 8+ the channel decides interruption — so M4-10 will
fail. And biometrics is still wired to nothing, below.
