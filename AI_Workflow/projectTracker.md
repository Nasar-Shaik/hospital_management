...
| Editions sell eight flags that gate no code                                                                | Re-measured 2026-08-20. Closed since: ~~`module.clinical.ris`~~ (2026-08-18), ~~`module.clinical.emergency`~~ (2026-08-19). Still open: `portal.patient` (sold in EVERY edition, nothing built), `module.finance.{opBilling,ipBilling,packages,preAuth}` — billing routes all gate on `module.ops.opd`, so a PLAN_HOSPITAL tenant gets Care Packages, a Hospital-Plus differentiator, for free — plus `module.analytics.groupDashboards`, `module.ops.interBranch`, `module.platform.multiEntity`, `platform.dedicatedDb`. Same class as a permission granted to nobody, one layer up. **Open.** |
