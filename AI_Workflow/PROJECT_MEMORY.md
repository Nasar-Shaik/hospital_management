...
- **Next priorities:** ① **Phase 1C — RBAC** (ADR-0010: permission catalog in `packages/permissions`, `permissions`/`rolePermissions`, the clinical role set, the three-layer `authorize` middleware [feature flag → permission → row scope], RBAC matrix suite). The seams are ready: `authorize` slots in after `authenticate`, the token already carries `roles`+`branchIds`, `rbac` already owns `roles`/`userRoles`. ② OpenAPI baseline (zod-to-openapi with the first business routes). ③ A6 Notifications — which unblocks forgot/reset-password, the one A3 endpoint deliberately not built (see §4).
