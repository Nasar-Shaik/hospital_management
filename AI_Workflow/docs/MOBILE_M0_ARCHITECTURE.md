...
| -------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime        | **React Native via Expo — SDK 54, pinned at M1**           | §2. Chosen over the then-latest 57 for a reason that only appears on a real phone: Expo Go serves one SDK per app version, the team's other Expo project is on 54, and the Play Store's Expo Go was 54.0.8 — so 57 could not be opened on a device at all. Still a supported SDK, so this is currency, not security. Temporary: revisit when the sibling app moves, or at M2, when the biometric gate forces a development build anyway. `apps/mobile/README.md` carries the standing warning against raising it alone. |
...
| What breaks on a bad value?                | Two different failures — see below. A well-shaped name that does not exist throws `RangeError` (a 500 on the server, **a render crash on a PHI screen** on the phone); an abbreviation is silently resolved to the WRONG zone, which is worse. |
...
1. **The client must never trust it.** Mobile resolves the display zone as
   `isValidZone(branch.timezone) ?? hospitalDefault` (`src/lib/time.ts`). Formatting a date must
   not be able to crash a ward list, whatever a hospital typed into a settings field two years ago.
2. **The server should stop accepting invalid zones.** One `.refine()` on the branch schema.
...
#### Corrected during M1: neither obvious validation works

This section originally proposed a `try { new Intl.DateTimeFormat(…) } catch` probe, or
`Intl.supportedValuesOf("timeZone")`. **Both were implemented, measured, and are wrong.** Recorded
here because backend item E must not repeat either mistake.

| Candidate check                                  | How it fails                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `new Intl.DateTimeFormat(_, { timeZone })`       | **Far too lenient.** Accepts `IST`, `EST`, `GMT` and `+05:30`. It does not merely accept them, it silently RESOLVES them: `EST` becomes `America/Panama`, a zone that never observes daylight saving, so a hospital that typed `EST` gets times that are right for half the year and an hour out for the other half. Nothing throws, nothing logs. |
| `Intl.supportedValuesOf("timeZone").includes(z)` | **Too strict, in the one place that matters here.** The list carries the legacy `Asia/Calcutta` and **not** `Asia/Kolkata` — so the modern, correct spelling of the zone every hospital on this platform sits in would be rejected as invalid.                                                                                                     |

The check that works is both together: require the **shape** of an IANA identifier —
`Area/Location`, which no abbreviation and no numeric offset has — and then let `Intl` reject a
well-shaped name that does not exist (`Asia/Kolkatta`). `UTC` is the one legitimate slash-free name
and is allowed explicitly. Implemented and tested in `apps/mobile/src/lib/time.ts`; item E should
use the same rule so the two ends agree.
...
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **M1** ✅ | **DELIVERED 2026-08-12.** Scaffold `apps/mobile` · dependency-cruiser boundary · api-client factory · secure storage · shared network/error foundation (§11) · app-lifecycle handler (§15) · hospital onboarding · login + MFA + refresh · logout-always-completes (§5) · `/auth/me` · permission tabs · light/dark · branch switcher. 79 foundation tests through the real client. | A signed-in app that opens on the right home | **none — and none was needed**                                                                 |
...
| **M3**    | Nurse: ward worklist · vitals capture · MAR administration · allergy check                                                                                                                                                                                                                                                                                                          | Observations recorded at the bed             | **FOUR GAPS — see `MOBILE_M3_NURSE_AUDIT.md`** (this cell said "none"; the audit disproved it) |
...
| **E** | **Validate `Branch.timezone` as an IANA identifier.** The field is documented as IANA and accepted as any 64-character string (§14). `IST` is stored happily and silently resolves to the wrong zone; a typo throws `RangeError`. One `.refine()` on the branch create/update schema **using the shape+`Intl` rule from §14 — not `supportedValuesOf`, which rejects `Asia/Kolkata`** — plus a data check for anything already stored.                    | Small — one schema refinement + a test                                                                             | **Before C, and before any timezone-dependent appointment work.** C resolves slots against `branch.timezone`, so it must not be reading an unvalidated field. |
