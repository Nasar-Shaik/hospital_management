...
| **Creates already guarded by a unique business key.** `roles` · `users` · `users/{id}/roles` · `tariff` · `packages` · `medicines` · `branches` · `departments` · `wards` · `rooms` · `beds` · `theatres` · `ot-bookings` · `ambulances` · `ambulance-trips` · `assets` · `lab-tests` · `mrd/icd-codes` · `mortuary/register` · `patients/{patientId}/allergies` · `encounters/{id}/discharge` · `encounters/{id}/transfer-bed` · `ot-bookings/{id}/operative-note`                                                                                                                      | 23    | A unique index refuses the second — `one_ward_name_per_branch`, `one_medicine_per_code`, `one_body_per_encounter`, `one_booking_per_theatre_start`, `one_trip_per_ambulance_start`, `one_active_allergy_per_allergen`, `one_discharge_summary_per_admission`, `one_open_stay_per_bed`, and the rest. The operation record's guard is a CONDITIONAL UPDATE rather than an index (`operativeNote: { $exists: false }`) because "already written" is a state of the row, not a second row — but it is the same argument: the write is already refused, so a key would be a second lock on a door already locked. A key would be a second lock on a door already locked. |
