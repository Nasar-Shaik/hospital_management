...
import type { SlotRef } from "./marAdminister";
...
/**
 * The identity to hand the confirmation screen — and the ONLY thing that travels.
 *
 * Not the drug name, not the dose, not the state. By the time a finger lands on a row the world
 * may have moved: another nurse may have answered that slot in the seconds since this list
 * rendered. S5A re-reads all of it from the server and matches on this triple.
 */
